...
function liste {
    ...
    # apps=$(git ls-remote --heads https://github.com/votre_compte/votre_repo.git | sed 's?.*refs/heads/??')
...
if [ "$choix" = "Installer tous les paquets" ]; then
    ...
    read -s -p $'\n'"veuillez saisir votre mot de passe : " user_password
    echo $''
    ...
    zenity --question --text "Téléchargement des fichiers depuis GitHub...\nConfirmez-vous l'installation des paquets suivants ?" --width 400 --height 200
    ...
    for package in $selected_packages; do
        if ls "$path_local/$package"*.deb >/dev/null 2>&1; then
            # Vérifier et installer les dépendances avant d'installer le paquet
            echo "$user_password" | sudo -S dpkg -i "$path_local/$package"*.deb
            if [ $? -ne 0 ]; then
                echo "$user_password" | sudo -S apt-get -f install -y
                echo "$user_password" | sudo -S dpkg -i "$path_local/$package"*.deb
            fi
